...
// brand name; the real model is set by REAL_MODEL (default: gpt-4o-mini), called
// through the OpenAI (ChatGPT) API.
//
...
const REAL_MODEL = process.env.REAL_MODEL || "gpt-4o-mini";
...
// Boot-safe: if the key isn't set yet, the service still starts green and the
// game just uses its offline parser until you add OPENAI_API_KEY in Render.
...
async function interpret(payload) {
  ...
  const userText = String(payload.prompt || "").slice(0, 400);
  const ctx = payload.context ? JSON.stringify(payload.context).slice(0, 2000) : "{}";
  const extraSystem = typeof payload.system === "string" ? payload.system.slice(0, 4000) : "";
  ...
  if (choice?.finish_reason === "content_filter") {
    return { reply: "GPT Luna 5.6 no puede construir eso. Prueba con otra cosa.", actions: [] };
  }
